...
○ 중첩 클래스
	...
	중첩 클래스란 클래스 안에 다른 클래스가 설계된어 있는 형태로
	클래스 내부에서만 사용할 보조클래스가 필요한 경우
	클래스를 중첩하여 프로그램의 구조를 보다 간단하고 알아보기 쉽도록 
	만들 수 있게 되는데 이러한 클래스를 중첩 클래스라고 한다.
...
○ 중첩 클래스의 종류(4가지)
	...
	2. 내부 클래스(inner class)
		...
		- static 중첩 클래스와 마찬가지로 프로그램 구조를 봐 더 간단하고
		  알아보기 쉽도록 할 수 있다
		- 외부 클래스의 메소드에서 내부 멤버 클래슬르 사용하기 위해서는
		  반드시 내부 멤버 클래스 인터페이스? 생성해 주어야 한다.
		- 외부 클래스의 멤버 변수와 메소드를
		  객체 생성 없이 바로 사용하는 것이 가능한다.
		- 내부 멤버 클래스는 외부에서 단독으로 객체를 생성하여 사용할 수 없다,
		  즉, 내부 멤버 클래스는 외브 클래스의 인스턴스 생성이 선챙되어야 한다는 것이다.
		- static으로 선언된변수 또는 메소드를 가질 수 없다.
	...
	4. 무명 클래스(익명 클래스, annonymous  class)
		...
		- 정의하는 부분과 생성하는 부분이 하나로 묶여져
		  new 수식이 있는 곳에서 바로 클래스 또는 인터페니스를 정의하는 것을 의미한다
...
class Test
{
	...
	public static class StaticNested					//--static
	{
		...
		void write()
		{
			...
			//System.out.println("b : "+b);				//--에러 발생			//★CHECK★
