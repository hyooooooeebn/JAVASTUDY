...
/*
 ...
 ○ 유니코드(unicode)
	...
	(USC : Universal Code System)를 말한다.
	...
	코드의 1문자 당 영어는 7비트, 비영어는 8비트, 한글이나 일본어는 16비트의 값을
	가지게 되는데 이들을 모두 16비트로 통일한 것이다.

 ○ UTF-8 과 UTF-16

 	UTF-8 과 UTF-16의 차이를 단 한마디로 표현하자면 
	...
	- ★ UTF-8(8bit UCS Transformation Format)
	  ...
	  1~6 바이트 가변장 멀티바이트로 변환한다.
	
	- ★ UTF-16(16bit UCS Transformation Format)
	  자바의 기본 코드.
	  자바에서는 문자 하나를 저장하면 바이트 수든, 영문자이든, 
	  한글문자이든 2바이트를 차지하게 된다.  
	...
	public static void main(String[] args) // throws UnsupportedEncodingException
	{
		...
		try
		{
			...
			for (byte b : buf )
				System.out.print("[" + b + "] ");
			System.out.println();
			// [-20] [-100] [-92] [-20] [-100] [-96] [-21] [-113] [-103]
			// ------------------ ------------------ -------------------
			...
			//	  문자열 객체 생성한 후 이름 출력 => 역순으로 진행해서 확인해보겠다.
			...
			// → 디폴트 캐릭터셋을(MS949) 기반으로 buf 바이트 배여을 합쳐 
			// 문자열 객체 생성한 후 이름 출력
			...
			// 인코딩 방식의 변환이 아니라 인코딩은 MS949로 하고 객체 생성을 UTF-8로 하는것. 
			// 신호부터 잘못된 거라서 글자 생성 자체가 망가짐.

			System.out.println("Value 3 : " + convert);
			// Value 3 : ?????? 
			// 지원되는 캐릭터셋으로 변환을 못해서. 표현할 방법이 없음을 의미
			...
			/*
			 ※ check
				...
				euc-kr 을 utf-8로 변환하겠다는 의도로 위와 같이 코딩을 해버리면
				이러한 변환으로 인해 바이트 배열이 깨져서 
				euc-kr 이든 utf-8 이든 어떠한 바이트 배열로 읽어오든
				깨져있는 것을 확인할 수 있다.
		...
		catch (UnsupportedEncodingException e)	// 문제가 생기면
		{
			System.out.println(e.toString());	// 이렇게 하겠다.
		}
